...
import { adminsIds, initialTags } from "../utils/helpers";
...
const CreateTemplate = () => {
  ...
  const [selectedTags, setSeletectedTags] = useState([]);
      ...
      uplaodTask.on(
        ...
        (error) => {
          if (error.message.includes("storage/unauthorized")) {
            toast.error(`Error:Authorized Revoked`);
          } else {
  ...
  const handleSelectedTags = (tag) => {
    if (selectedTags.includes(tag)) {
      setSeletectedTags(selectedTags.filter((selected) => selected !== tag));
    } else {
      setSeletectedTags([...selectedTags, tag]);
    }
  };
  ...
  const removeTemplate=async(template)=>{
    ...
    await deleteObject(deleteRef).then(async()=>{
      await deleteDoc(doc(db,"templates",template._id)).then(()=>{
        toast.success("Template deleted successfully from the cloud")
        templatesRefetch()
      })
      .catch((err)=>{
        toast.error(`Erroe:${err.message}`)
      })
    })
  }

  useEffect(()=>{
    if(!templatesIsLoading && !adminsIds.includes(user?.uid)){
      navigate("/",{replace:true})
    }
  },[user,templatesIsLoading])


  return (
    <div className="w-full px-4 lg:px-10 2xl:px-32 py-4 grid grid-cols-1 lg:grid-cols-12">
      {/* left */}
      <div className="col-span-12 lg:col-span-4 2xl:col-span-4 w-full flex-1 flex items-center justify-start flex-col gap-4 px-2">
        ...
        {/* file upload */}
        <div className="w-full bg-gray-300 backdrop-blur-md h-[420px] lg:h-[620px] 2xl:h-[740px] rounded-md border-2 border-dotted border-gray-300 cursor-pointer flex items-center justify-center">
              ...
              {!imageAsset?.uri ? (
                <React.Fragment>
                  <label className="w-full cursor-pointer h-full">
                    <div className="flex flx-col items-center justify-center h-full w-full">
      ...
      {/* right */}
      <div className="col-span-12 lg:col-span-8 2xl:col-span-8 px-2 w-full flex-1 py-4">
        {templatesIsLoading ? (
          <React.Fragment>
          ...
          <React.Fragment>
            ...
            {templates && templates.length>0 ? (
              <React.Fragment>
               ...
               {templates?.map((template)=>(
                 <div key={template._id} className="w-full h-[500px] rounded-md overflow-hidden relative">
                  <img src={template?.imageURL} className="w-full h-[400px] object-cover" alt="template" />
              ...
              <React.Fragment>
                <div className="w-full h-full flex flex-col gap-6 items-center jistify-center">
                  <PuffLoader color="#498FCD" size={40} />
