  ...
  return (
    <div className="w-full flex flex-col items-center justify-start py-12">
      <div className="w-full h-72 bg-blue-50">
        <img
          src="https://www.thetechlounge.com/wp-content/uploads/2023/01/spilled-coffee-on-laptop.jpg"
          alt=""
        ...
        <div className="flex items-center justify-center flex-col gap-4">
          {user?.photoURL ? (
            ...
            <React.Fragment>
              <img
                src={
                  "https://img.freepik.com/premium-vector/adorable-cyberpunk-dj-vector_868778-421.jpg?w=1060"
                }
                className="w-24 h-24 rounded-full border-2 border-white -mt-12 shadow-md"
                referrerPolicy="no-referrer"
                loading="lazy"
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-2 px-4 py-6">
          <AnimatePresence>
           {activeTab==="collections" && (<React.Fragment>
            {user?.collections?.length>0 && user?.collections ? (<RenderATemplate templates={templates?.filter((temp)=>user?.collections?.includes(temp?._id))} />
            ):(
           ...
           {activeTab==="resumes" && (<React.Fragment>
            {savedResumes?.length>0 && savedResumes ? (<RenderATemplate templates={savedResumes} />
            ):(
              <div className="col-span-12 w-full flex flex-col items-center justify-center gap-3">
                <img src={NoData}
                className="w-32 h-auto object-contain"
